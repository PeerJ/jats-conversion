...
echo "creating CrossRef DOI deposition"
xsltproc --catalogs \
    --nodtdattr \
  ...
  --stringparam 'nextVersionDoi' '10.7728/nextVersionDoi' \
	"$XSL/jats-to-unixref-posted-content-preprint.xsl" "$ARTICLE" > "$OUTPUT_DIR/crossref.xml"

echo "Validating for CrossRef DOI deposition"
xmllint --nonet $TRACE --noout --schema "$RESOURCES/crossref/crossref4.4.1.xsd" "$OUTPUT_DIR/crossref.xml"
